	...
	HWND hWnd, bool fullScreen, float screenDepth, float screenNear)
{
	...
	//	++index;		
	//}
	//for (int index = 0; index < vAdapters.size(); index++)
	//{
	//	vAdapters[index]->GetDesc(&adapterDesc);		
	//	MessageBox(hWnd, adapterDesc.Description, L"info", MB_OK);
	//}
	...
	//error = wcstombs_s()
	error = wcstombs(m_videoCardDescription, adapterDesc.Description, 128);
	if (error == 0)
	{
		return false;
	}
