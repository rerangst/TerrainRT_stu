...
bool UserInterfaceClass::Initialize(D3dClass* Direct3D, int screenHeight, int screenWidth)
{
	...
	//result = m_VideoStrings->UpdateSentence(Direct3D->GetDeviceContext(), m_Font1, memoryString, 10, 30, 1.0f, 1.0f, 1.0f);
	if (!result)
	{
		return false;
	}
...
bool UserInterfaceClass::Frame(ID3D11DeviceContext* deviceContext, int fps, 
...
bool UserInterfaceClass::Render(D3dClass* Direct3D, ShaderManagerClass* ShaderManager, 
